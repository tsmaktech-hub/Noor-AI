...
export const SavedView: React.FC<SavedViewProps> = ({
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 font-sans text-zinc-900 dark:text-zinc-100 transition-colors">
      ...
      {/* Header */}
      <div className="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 p-6 sm:p-8 rounded-2xl shadow-sm mb-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 transition-colors">
        <div>
        ...
        <div className="relative w-full sm:w-64">
          ...
          <input
            ...
            className="w-full bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 focus:border-zinc-900 dark:focus:border-zinc-100 rounded-xl py-2 pl-9 pr-3 text-xs text-zinc-900 dark:text-white placeholder-zinc-400 dark:placeholder-zinc-500 focus:outline-none"
          />
        </div>
      </div>
      ...
      {savedEvidences.length === 0 ? (
        <div className="text-center py-16 bg-white dark:bg-zinc-950/60 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-8">
          <Bookmark className="w-12 h-12 text-zinc-400 mx-auto mb-3" />
          <h2 className="text-lg font-bold text-zinc-700 dark:text-zinc-300">No Saved Evidences Yet</h2>
          <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-1 max-w-md mx-auto">
            When you ask questions in Islamic AI or explore topics, click "Save Answer" or "Save" to bookmark them here for quick reference.
          </p>
        </div>
      ) : filtered.length === 0 ? (
        <div className="text-center py-12 bg-white dark:bg-zinc-950/60 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-6">
        ...
        <div className="space-y-8">
          {filtered.map((item) => (
            <div
              key={item.id}
              className="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-6 shadow-sm relative transition-colors"
            >
              {/* Header Ribbon */}
              <div className="flex items-center justify-between pb-3 border-b border-zinc-200 dark:border-zinc-800 mb-4">
                ...
                <div className="flex items-center space-x-2 shrink-0">
                  ...
                  <button
                    ...
                    className="p-2 text-rose-600 dark:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-950/60 rounded-lg transition-colors"
              ...
              {/* Direct Answer */}
              <div className="bg-zinc-50 dark:bg-zinc-900/90 p-4 rounded-xl border border-zinc-200 dark:border-zinc-800 mb-6">
